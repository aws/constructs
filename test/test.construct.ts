...
export = {
  ...
  'fails if context key contains unresolved tokens'(test: Test) {
    ...
    test.throws(() => root.node.setContext(`my-${token}`, 'foo'), /Invalid context key/);
    test.throws(() => root.node.tryGetContext(token), /Invalid context key/);
    test.done();
  },
  ...
  'addWarning(message) can be used to add a "WARNING" message entry to the construct'(test: Test) {
    ...
    test.deepEqual(con.node.metadata[0].type, ConstructMetadata.WARNING_METADATA_KEY);
  ...
  'addError(message) can be used to add a "ERROR" message entry to the construct'(test: Test) {
    ...
    test.deepEqual(con.node.metadata[0].type, ConstructMetadata.ERROR_METADATA_KEY);
  ...
  'addInfo(message) can be used to add an "INFO" message entry to the construct'(test: Test) {
    ...
    test.deepEqual(con.node.metadata[0].type, ConstructMetadata.INFO_METADATA_KEY);
